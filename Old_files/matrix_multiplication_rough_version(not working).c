...
double compare_governors(char* matrix_size) {
	...
	if (pid == -1) {
		...
		FILE *file_start_flag = fopen("Measurement_start_flag", "w");
		fprintf(file_start_flag, "start");
		char cmd_buf[1024];
		snprintf(cmd_buf, sizeof(cmd_buf), "matrix_%s.txt", matrix_size);
		if(execl("matrix_input", "matrix_input", cmd_buf, NULL) == -1) {
			fprintf(stderr, "Cannot run matrix_input: %s\n", strerror(errno));
			return 1;
		}
		system(cmd_buf);
		printf("In child process: finished running matrix_input\n");		
	} //end of else if
		...
		do {
			fscanf(file_start_flag, "%s", start_flag);
			printf("wait!\n");
			usleep(1000);
		} while (!strcmp(start_flag, "start"));
		...
		while(!WIFEXITED(status) && isrunning) {
			//Put timestamp
			gettimeofday(&tv, NULL);
			...
			printf("child exit: %d\t\t", WIFEXITED(status));
			printf("%d energy sum: %f for %dMHz\n", n, energy_sum, freq_val_int);
			n++;
			pid = waitpid(-1, &status, WNOHANG);
		} //end of while loop
		printf("End of loop\n");
