...
int main(int arc, char* argv[]) {
	...
	printf("Rough Power Model-based results: \n");
	...
	//Continue on adaptive power model!!
	printf("\n\nAdaptive Model-base Results: \n");

	pid_t pid = fork();
	double new_aC_little = 0;
	double new_aC_big = 0;

	system("cpufreq-set -g userspace");
	//start from 250MHZ
	char cmd_buf[1024];		
	system("cpufreq-set -f 400MHz");
	if (pid == 0) {
		system("dd if=/dev/zero of=/dev/null count=10000000");
		printf("!!! dd finished running !!!\n\n");
	}
	else {
		/*
		//printf("here!\n");
		for(i=0;i<8;i++) {
			printf("i: %d\n", i);
			snprintf(cmd_buf, sizeof(cmd_buf), "cpufreq-set -f %d", freq_set_MHz[i]);
			system(cmd_buf);
			power_model();
			new_aC_little += aC;
		}
		...
		*/
		...
		new_aC_little = aC;
		system("cpufreq-set -f 1.2GHz");
		sleep(1);
		power_model();
		//printf("power measured: %f\n", power_measured);
		new_aC_big = aC;
		system("cpufreq-set -g interactive");
		printf("new aC (little): %f\n",new_aC_little);
		printf("new aC (big): %f\n", new_aC_big);
		...
		for (i=i_QoS; i<17; i++) {
			if (i <= 7) {//little CPU (up to 600MHz)
				if (i < 3) 
					...
					voltage = 1.2 * 0.001 * freq_set_MHz[i] + 0.4765;
				power = (new_aC_little * voltage * voltage * freq_set_MHz[i]) + (6.534 * 0.00001);
			}
			if (i > 7) { //big CPU (800MHz - 1.6GHz)
				voltage = 3.152 * 0.0001 * freq_set_MHz[i] + 0.647287;
				power = (new_aC_big * voltage * voltage * freq_set_MHz[i]) - (5.29 * 0.01);
			}
			energy_array[i] = exe_t_array[i] * power;
			printf("Execution time: %fs\t", exe_t_array[i]);
			printf("Power: %fW\t\t", power);
			printf("Energy: %f Watt-second for %dMHz\n", energy_array[i], freq_set_MHz[i]);
		}
